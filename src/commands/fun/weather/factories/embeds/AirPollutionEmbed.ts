import { WeatherEmojis, WeatherLinkSchema, OpenWeatherAPI, GeocodeData, AirPollutionData, WeatherHandler } from '../../../../..';
import { HandlerContext, HandlerEmbed } from 'discord.js-commands';
import { Util } from 'discord.js';

export class AirPollutionEmbed extends HandlerEmbed {

    constructor(handler: WeatherHandler, context: HandlerContext, geocode: GeocodeData | WeatherLinkSchema, airPollution: AirPollutionData) {
        ...
        this.addField('Quantity', (
            `${WeatherEmojis.getQualityEmoji(context.client, this.getCO(components.co))} ${components.co} μg/m³\n` +
            `${WeatherEmojis.getQualityEmoji(context.client, this.getNO(components.no))} ${components.no} μg/m³\n` +
            `${WeatherEmojis.getQualityEmoji(context.client, this.getNO2(components.no2))} ${components.no2} μg/m³\n` +
            `${WeatherEmojis.getQualityEmoji(context.client, this.getO3(components.o3))} ${components.o3} μg/m³\n` +
            `${WeatherEmojis.getQualityEmoji(context.client, this.getSO2(components.so2))} ${components.so2} μg/m³\n` +
            `${WeatherEmojis.getQualityEmoji(context.client, this.getPM2_5(components.pm2_5))} ${components.pm2_5} μg/m³\n` +
            `${WeatherEmojis.getQualityEmoji(context.client, this.getPM10(components.pm10))} ${components.pm10} μg/m³\n` +
            `${WeatherEmojis.getQualityEmoji(context.client, this.getNH3(components.nh3))} ${components.nh3} μg/m³\n`
        ), true)
    }
