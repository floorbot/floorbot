...
import { InteractionReplyOptions, MessageActionRow, ButtonInteraction, GuildMember, CommandInteraction, TextChannel, Message, Permissions, Guild, GuildChannel, SelectMenuInteraction, MessageSelectMenu } from 'discord.js';
import { CommandClient, HandlerContext, HandlerCustomData, HandlerResult, HandlerEmbed } from 'discord.js-commands';
import { PoolConfig } from 'mariadb';

import * as nconf from 'nconf';
import { GuildHandlerGroup, GuildHandler } from '../../..';
...
export class WeatherHandler extends GuildHandler<WeatherCustomData> {
    ...
    constructor(poolConfig: PoolConfig) {
        super({ commandData: WeatherCommandData, group: GuildHandlerGroup.FUN, id: 'weather', permissions: [Permissions.FLAGS.ADMINISTRATOR] });
    ...
    public override async isAuthorised(context: HandlerContext, customData?: HandlerCustomData): Promise<string[]> {
        if (context instanceof CommandInteraction) {
            ...
            if ([WeatherSubCommandName.LINK, WeatherSubCommandName.UNLINK].includes(<WeatherSubCommandName>subCommand)) {
                ...
                if (targetMember !== member) return super.isAuthorised(context, customData)
            }
        }
        return [];
    }
    ...
    public override async onSelectMenu(interaction: SelectMenuInteraction, _customData: HandlerCustomData): Promise<any> {
        ...
        const selectMenu = <MessageSelectMenu>message.components[0]!.components[0];
    ...
    public override getEmbedTemplate(context: HandlerContext, customData?: WeatherCustomData): HandlerEmbed {
    ...
    private async fetchWarningResponse(context: HandlerContext, scope: GuildMember | LocationData | null): Promise<InteractionReplyOptions> {
        ...
        if (!location && !link) return WeatherEmbedFactory.getMissingParamsEmbed(this, context, member).toReplyOptions();
        const geocoding = location ? await this.api.geocoding(location) : [link!];
        if (OpenWeatherAPI.isError(geocoding)) return WeatherEmbedFactory.getAPIErrorEmbed(this, context, geocoding).toReplyOptions();
        if (!geocoding.length) return WeatherEmbedFactory.getUnknownLocationEmbed(this, context, location!).toReplyOptions();
        const onecall = await this.api.oneCall(geocoding[0]!);
        if (OpenWeatherAPI.isError(onecall)) return WeatherEmbedFactory.getAPIErrorEmbed(this, context, onecall).toReplyOptions();
        const embed = WeatherEmbedFactory.getAlertEmbed(this, context, geocoding[0]!, onecall);
    ...
    private async fetchCurrentResponse(context: HandlerContext, scope: GuildMember | LocationData | null): Promise<InteractionReplyOptions> {
        ...
        const onecall = await this.api.oneCall(geocoding[0]!);
        if (OpenWeatherAPI.isError(onecall)) return WeatherEmbedFactory.getAPIErrorEmbed(this, context, onecall).toReplyOptions();
        const embed = WeatherEmbedFactory.getCurrentEmbed(this, context, geocoding[0]!, onecall);
        const actionRow: MessageActionRow = new MessageActionRow().addComponents([
            WeatherButtonFactory.getWeatherButton(this, WeatherDisplayType.FORECAST, geocoding[0]!),
            WeatherButtonFactory.getWeatherButton(this, WeatherDisplayType.AIR_QUALITY, geocoding[0]!),
    ...
    private async fetchForecastResponse(context: HandlerContext, scope: GuildMember | LocationData | null): Promise<InteractionReplyOptions> {
        ...
        const onecall = await this.api.oneCall(geocoding[0]!);
        if (OpenWeatherAPI.isError(onecall)) return WeatherEmbedFactory.getAPIErrorEmbed(this, context, onecall).toReplyOptions();
        const embed = WeatherEmbedFactory.getForecastEmbed(this, context, geocoding[0]!, onecall);
        const actionRow: MessageActionRow = new MessageActionRow().addComponents([
            WeatherButtonFactory.getWeatherButton(this, WeatherDisplayType.CURRENT, geocoding[0]!),
            WeatherButtonFactory.getWeatherButton(this, WeatherDisplayType.AIR_QUALITY, geocoding[0]!),
            ...(onecall.alerts && onecall.alerts.length ? [WeatherButtonFactory.getWeatherButton(this, WeatherDisplayType.WARNING, geocoding[0]!)] : []),
    ...
    private async fetchAirQualityResponse(context: HandlerContext, scope: GuildMember | LocationData | null): Promise<InteractionReplyOptions> {
        ...
        if (OpenWeatherAPI.isError(airPollution)) return WeatherEmbedFactory.getAPIErrorEmbed(this, context, airPollution).toReplyOptions();
        const embed = WeatherEmbedFactory.getAirPollutionEmbed(this, context, geocoding[0]!, airPollution);
        const actionRow: MessageActionRow = new MessageActionRow().addComponents([
            WeatherButtonFactory.getWeatherButton(this, WeatherDisplayType.CURRENT, geocoding[0]!),
            WeatherButtonFactory.getWeatherButton(this, WeatherDisplayType.FORECAST, geocoding[0]!),
            WeatherButtonFactory.getViewMapButton(this, geocoding[0]!)
    ...
    private async fetchServerTempsResponse(context: HandlerContext, viewData: { page: number, perPage?: number, order?: WeatherTempsOrder }): Promise<InteractionReplyOptions> {
        ...
        if (!links.length) return WeatherEmbedFactory.getNoLinkedMembersEmbed(this, context, channel).toReplyOptions();
        ...
        const embed = WeatherEmbedFactory.orderEmbedData(WeatherEmbedFactory.getServerTempsEmbed(this, context, sliced), order);
    ...
    private async fetchLinkResponse(context: HandlerContext, location: LocationData, targetMember: GuildMember) {
        ...
        const embed = WeatherEmbedFactory.getLinkedEmbed(this, context, geoLocation, targetMember);
    ...
    private async fetchUnlinkResponse(context: HandlerContext, targetMember: GuildMember): Promise<InteractionReplyOptions> {
        ...
        const embed = WeatherEmbedFactory.getUnlinkedEmbed(this, context, member)
