...
export class SafebooruAPI {
    ...
    private static async request<T>(endpoint: string, params: Map<string, string>, options: any = {}): Promise<T> {
        ...
        if (!options.headers || !options.headers.Authorization) {
            ...
            const username = process.env['SAFEBOORU_USERNAME'];
            const apiKey = process.env['SAFEBOORU_API_KEY'];
            if (username && apiKey) {
    ...
    public static async autocomplete(tag: string = String(), limit: number = 10): Promise<SafebooruAPIAutocomplete[]> {
