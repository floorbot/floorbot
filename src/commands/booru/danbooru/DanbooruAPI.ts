...
export class DanbooruAPI {
    ...
    private static async request<T>(endpoint: string, params: Map<string, string>, options: any = {}): Promise<T> {
        ...
        const url: string = `https://danbooru.donmai.us/${endpoint}.json?${paramString}`;
        return fetch(url, options).then((res: any) => res.json());
    }

    public static async count(tags: string = String()): Promise<DanbooruAPICount | DanbooruAPIError> {
        return DanbooruAPI.request('counts/posts', new Map([['tags', tags!]]));
    }

    public static async random(tags: string = String()): Promise<DanbooruAPIPost | DanbooruAPIError> {
        return DanbooruAPI.request('posts/random', new Map([['tags', tags!]]));
    }

    public static async autocomplete(tag: string = String(), limit: number = 10): Promise<DanbooruAPIAutocomplete[]> {
        return DanbooruAPI.request('autocomplete', new Map([
            ['search[query]', tag],
    ...
    public static async get404(): Promise<string> {
        const data = await DanbooruAPI.random('pool:16069');
...
export interface DanbooruAPICount {
    readonly counts: {
        readonly posts: number
    }
}

export interface DanbooruAPIAutocomplete {
