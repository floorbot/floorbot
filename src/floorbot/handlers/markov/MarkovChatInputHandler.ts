...
import { MarkovStateTable } from '../../../app/tables/MarkovStateTable.js';
import { ChatInputCommandInteraction, MessageComponentInteraction } from 'discord.js';
import { Pool } from 'mariadb';
import { MarkovChannelTable } from '../../../app/tables/MarkovChannelTable.js';
import { MarkovReplyBuilder } from './MarkovReplyBuilder.js';
...
export class MarkovChatInputCommandHandler extends ChatInputCommandHandler {
    ...
    constructor({ pool }: { pool: Pool; }) {
        ...
        this.channelTable = new MarkovChannelTable(pool);
    ...
    public async run(command: ChatInputCommandInteraction): Promise<any> {
        ...
        let settings = await this.channelTable.selectChannel({ guildId: command.guildId, channelId: command.channelId });
        const replyOptions = new MarkovReplyBuilder(command).addControlPanelReply({ settings });
        const message = await command.followUp(replyOptions);
        const collector = Util.createComponentCollector(command.client, message);
        collector.on('collect', (component: MessageComponentInteraction) => {
    ...
    public override async setup(client: HandlerClient): Promise<void> {
        ...
        return await super.setup(client);
    }
}
